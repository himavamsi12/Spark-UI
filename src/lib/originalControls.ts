...
export const ORIGINALS: OriginalEntry[] = [
  ...
  },
  {
    ...
    category: "Navigation",
    blurb: "Switching between pages pinches the hero into a thin diamond slit, drops it away, then reopens the slit into the next page's full-bleed image.",
    features: [
      "The transition uses a single clip-path polygon animated from a full rectangle down to a zero-height diamond and back",
      "Page content is swapped mid-timeline, at the exact moment the clip-path is fully closed, so the swap is invisible",
      "Built as an internal page switcher so the transition can be triggered and replayed without real route navigation",
    ],
    controls: [
      ...
      { key: "brand", label: "Brand Name", type: "text", default: "Aethos", description: "Wordmark shown in the nav bar." },
      { key: "fontFamily", label: "Font", type: "font", default: "var(--font-dm-sans), sans-serif", description: "Typeface used for the nav and page title." },
